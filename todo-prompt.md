...
완료 후: 루트 todo.md의 1번 항목(카탈로그 상품 목록 API)을 체크([ ] → [x])로 갱신한다.
```
...
완료 후: 루트 todo.md의 2번 항목(주문 이행 전이 API)을 체크([ ] → [x])로 갱신한다.
```
...
완료 기준: 위 테스트가 통과하고 ./gradlew build 게이트가 통과한다.
완료 후: 루트 todo.md의 3번 항목(재고 운영 API)을 체크([ ] → [x])로 갱신한다.
```
...
완료 기준: 위 테스트가 통과하고 ./gradlew build 게이트가 통과한다.
완료 후: 루트 todo.md의 4번 항목(추가 변형 등록 + 변형 관리 API)을 체크([ ] → [x])로 갱신한다.
```
...
완료 기준: 위 테스트가 통과하고 ./gradlew build 게이트가 통과한다.
완료 후: 루트 todo.md의 5번 항목(상품 관리 API)을 체크([ ] → [x])로 갱신한다.
```
...
완료 기준: 위 테스트가 통과하고 ./gradlew build 게이트가 통과한다.
완료 후: 루트 todo.md의 6번 항목(회원 관리 API)을 체크([ ] → [x])로 갱신한다.
```
...
완료 기준: 위 테스트가 통과하고 ./gradlew build 게이트가 통과한다.
완료 후: 루트 todo.md의 7번 항목(쿠폰 정책 전환 + 내 발급 쿠폰 목록 API)을 체크([ ] → [x])로 갱신한다.
```
...
완료 기준: 위 테스트가 통과하고 ./gradlew build 게이트가 통과한다.
완료 후: 루트 todo.md의 8번 항목(결제 거래 추적 노출)을 체크([ ] → [x])로 갱신한다.
```
...
완료 후: 루트 todo.md의 9번 항목(로컬 실행 구성 + README)을 체크([ ] → [x])로 갱신한다.
```
...
완료 후: 루트 todo.md의 10번 항목(문서 현행화(멱등 필터))을 체크([ ] → [x])로 갱신한다.
```
