...
- **취소×출고 경합 회복 — 출고 차단**(#24, 2026-07-17 확정): 취소 개시 마커를 PG 환불 앞에 커밋해 취소 진행 중 주문의 `ship`을 거부한다 — 환불 고아(결제 CANCELLED × 주문 PAID+SHIPPED)를 회복이 아니라 원천 차단한다. `settleRecorded`의 "CANCELLED 결제 잔여는 취소 재시도 소유" 설계가 그대로 유효해진다(재시도가 SHIPPED에 막히지 않음). 재청구(환불 취소 없는 PG 모델·재인증 필요로 비현실), SHIPPED 취소 허용(이행 의미론 충돌), 운영 대사 명시(코드 해소 우선 결정 패턴과 불일치)는 기각했다.
...
- 상태: 완료
- 결정(2026-07-17 확정): 출고 차단(위 선행 결정) — 취소 개시 마커를 PG 환불 앞에 커밋해 취소 진행 중 주문의 ship을 거부.
- 완료 메모(2026-07-17): `Order.cancelRequestedAt` 마커(멱등 재개시 no-op) + ship의 409 `ORDER_CANCEL_IN_PROGRESS` 거부. 마커 커밋 vs ship 커밋은 `@Version` 직렬화, 복원 게이트(PAID→CANCELLED 1회 전이)는 무변경 — 독립 리뷰가 복원 0회·2회 인터리빙 부재와 "CANCELLED×SHIPPED 도달 경로 없음"을 전수 확인. 마커 잔여는 취소 재시도 소유(출고 차단으로 재시도가 항상 완결 도달). 잔여 — PG 환불 영구 실패 + 무재시도 극단에서 주문이 PAID+출고 차단으로 남음(관리자 마커 해제 오퍼레이션은 범위 밖, 필요 시 별도 항목).
