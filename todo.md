...
- 상태: 보류(결정 대기)
- 대기 중인 결정: 아래 "선행 결정"의 두 안 중 하나를 확정해야 착수할 수 있다. 용어를 고르는 일이라 코드에서 답이 나오지 않고, 어느 쪽을 고르든 이미 머지된 슬라이스의 표현을 바꾸므로 임의로 정하지 않았다.
...
- 선행 결정 필요 (둘 중 하나):
  - **안 A — 용어집을 따른다**: 코드 11곳의 `논리삭제`를 `소프트삭제`로 바꾼다. 용어집이 용어의 단일 출처라는 원칙에 맞고 `docs/`(architecture·entity-persistence)와도 일치하지만, 변경 지점이 많고 그중 일부는 `@Operation`·`@Schema`의 API 문서 문구다.
  - **안 B — 코드를 따른다**: 용어집 `soft delete` 행을 `논리삭제`로 고치고 `DOMAIN_MODEL.md:135`가 한 줄에서 두 표기를 섞어 쓰는 것만 정리한다. 변경이 1~2곳으로 작지만 `docs/`의 `소프트삭제` 용례와 어긋나 그쪽도 함께 봐야 한다.
- 같은 결정이 필요한 두 번째 용어: `issued coupon`의 등재어는 `발급분`인데 app-api Swagger 15곳 이상이 "발급 쿠폰"을 쓴다. 안 A/B와 같은 방향으로 함께 정한다.
