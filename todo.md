...
### 3. 잔여 규칙 정합 sweep
- 상태: 완료
- 완료 메모: 4개 카테고리 전수 스캔 후 발산만 외과적으로 정정(9파일, +5/−12, 주석만). C1 이름 되풀이 요약 5건 제거(계약은 타입 doc이 소유해 유실 없음). C2 private 상수 근거 2건을 `//`로. C3 `@return` 1건 흡수·자명 `@param` 1건 제거(전 모듈 `@return` 잔여 0). C4 순수 `id desc` 쿼리군만 UUIDv7 근거 주석 보강(`ProductRepository.findExposedPage`) — 관측된 `OrderRepository`는 `createdAt desc, id desc` 이종 정렬이라 근거가 부정확해 미추가(`IssuedCouponRepository`와 무주석 일관). 세션 독립 리뷰가 카테고리별 잔여 0을 전수 확인.
