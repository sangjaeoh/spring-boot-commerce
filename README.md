...
Spring Boot 기반 커머스 최소 구현 연습 프로젝트다. 7개 도메인(회원·상품·재고·장바구니·쿠폰·주문·결제)을 모듈러 모놀리스로 두되, 도메인 간은 ID 값 참조 + 앱 계층(파사드) 조율로만 잇는다(물리 FK·크로스 도메인 컴파일 의존 없음). 결제는 동기 stub PG, 인증·로그인은 범위 밖이다.
...
| `module-common` | `common-core` · `common-jpa` · `common-messaging` · `common-web` | 공용 값 객체, JPA 기반(감사·마이그레이션 팩토리), 이벤트 발행 계약, 웹 경계(problem+json 핸들러·멱등 필터) |
| `module-external` | `external-payment` | 결제 포트의 동기 stub PG 어댑터 |
| `module-infra` | `infra-messaging` | in-process 이벤트 발행 transport |
...
요구 도구: JDK 25, Docker(Compose v2). 스모크 테스트는 `curl`·`jq`를 쓴다.

1. PostgreSQL을 띄운다(호스트 포트 55432 — 로컬 기존 PostgreSQL과 충돌하지 않게 비표준 포트를 쓴다).
...
MEMBER_ID=$(curl -s -X POST $BASE/members -H 'Content-Type: application/json' \
  -d '{"email":"buyer@example.com","name":"구매자"}' | jq -r .memberId)

# 2. 상품 등록(첫 변형·초기 재고 시딩 포함) 후 변형 ID 조회
...
통합 테스트는 Testcontainers가 PostgreSQL을 직접 띄우므로 `docker compose` 기동 없이 돈다(Docker 데몬만 필요). 빌드에 Spotless·NullAway·Error Prone·ArchUnit 게이트가 포함된다 — [`docs/code-quality.md`](docs/code-quality.md).
