...
| cancelRequestedAt | Instant | 선택 | 취소 개시 마커(`requestCancellation` 세팅, 재개시 no-op). 마커가 있는 동안 이행 서비스가 출고를 거부한다. `Cancellation` 임베더블 컴포넌트 |
| cancelledAt | Instant | 선택 | 취소 시각(`cancel` 세팅). `Cancellation` 임베더블 컴포넌트 |
| cancellationReason | CancellationReason | 선택 | 취소 사유(`cancel` 세팅). `Cancellation` 임베더블 컴포넌트 |
...
| returnStatus | ReturnStatus | 선택 | 주문 단위 반품 요청 축(아래 상태표). 요청이 없으면 없음. `ReturnRequest` 임베더블 컴포넌트 |
| returnRequestedAt | Instant | 선택 | 반품 요청 시각(`requestReturn` 세팅). `ReturnRequest` 임베더블 컴포넌트 |
| returnReason | RefundReason | 선택 | 반품 요청 사유(`requestReturn` 세팅). 승인되면 이 사유로 `refund`가 실행된다. `ReturnRequest` 임베더블 컴포넌트 |
...
| holdReason | HoldReason | 선택 | 이행 보류 사유. ON_HOLD에서만 존재(`hold` 세팅, `release` clear) |
...
- 생성: 결제 완료(`OrderModifier.markPaid`)가 `Fulfillment.create(orderId)`까지 한 트랜잭션에서 원자적으로 처리한다 — 결제 완료와 이행 생성은 같은 유스케이스의 단일 사건이라 "같은 도메인 모듈 내 다중 애그리거트 조율" 예외(architecture.md)를 적용했다. 이벤트 기반 생성(별도 소비자)은 쓰지 않는다 — 아웃박스 릴레이가 `app-batch`에서만 켜져 있어(§아웃박스) 실시간성을 보장할 수 없었기 때문이다.
...
주문 결제와 부분 환불 거래 기록을 소유한다. 외부 PG 연동은 도메인이 소유한 벤더 중립 포트(`PaymentGateway`)로 하고, 실제 구현은 외부 어댑터가 담당한다.

- 애그리거트 루트: `Payment`, `PaymentRefund`(부분 환불 거래 기록 — Payment를 논리 참조하는 독립 루트, 자식 없음)
- 스키마/테이블: `payment` / `payment`, `payment_refund`
...
|---|---|---|---|
| id | UUID | 필수 | PK, UUIDv7 |
| paymentId | UUID | 필수 | 결제 논리 참조(FK 없음) |
| amount | Money | 필수 | 이번 부분 환불분 금액 |
| pgCancelTransactionId | String | 선택 | PG 부분 취소 거래 ID. 무 PG 승인·0원 환불은 PG 호출이 없어 없음 |
| refundKey | UUID | 필수 | 라인 멱등 키(주문 라인 ID). 유니크 — 같은 라인 재시도는 행을 중복 생성하지 않는다 |
...
| cancelPartially | paymentId, amount(이번 환불분), cumulativeTotal(환불 누계), refundKey | APPROVED에서 `refundedAmount`를 `cumulativeTotal`로 단조 동기화(+PG 부분 환불 호출, `amount==0`이거나 PG 미호출 승인이면 생략); 누계가 결제액에 도달하면 CANCELLED 전이(+`cancelledAt`); `PaymentRefund` 행 기록(`refundKey` 유니크 — 재시도 중복 기록 없음); 이미 CANCELLED면 조용히 통과 | 미존재; 잘못된 전이(APPROVED 아님); 누계가 결제액 초과; 낙관락 충돌 |
...
|---|---|---|---|
| id | UUID | 필수 | PK, UUIDv7 |
| eventId | UUID | 필수 | 처리한 `StockRestocked` 이벤트 식별자. (eventId, memberId) 조합 유니크(멱등 가드) |
| memberId | UUID | 필수 | 발송 대상 회원 논리 참조(FK 없음). (eventId, memberId) 조합 유니크. V3 마이그레이션 이전 이벤트 단위 레거시 기록은 NULL |
| createdAt | Instant | 필수 | 생성 시각(Auditing) |
| updatedAt | Instant | 필수 | 수정 시각(Auditing) |
...
- `RestockNotification`은 (이벤트, 회원) 단위 처리 기록이다(`event_id`·`member_id` 조합 유니크) — 발송에 성공한 회원만 기록하므로, 같은 이벤트가 재전달되거나 일부 회원 발송 실패 후 재시도돼도 이미 발송받은 회원은 건너뛰고 미발송 회원에게만 발송한다. V3 마이그레이션 이전 이벤트 단위 레거시 기록(`member_id` NULL)은 이미 처리 완료된 이벤트의 흔적으로 남고 신규 로직과 충돌하지 않는다.
- 재입고 알림 소비(`StockRestockedListener`, `StockRestocked` 소비): (1) 변형 → 상품(productId·상품명) 해석. (2) 그 상품을 찜한 회원 중 미탈퇴 회원(정지 회원 포함, 탈퇴 회원 제외) 목록 조회. (3) 회원마다 (eventId, memberId) 처리 기록 존재 여부로 멱등 가드 — 있으면 스킵, 없으면 이메일로 상품명을 담은 재입고 메일 발송 후 처리 기록(`RestockNotification`) 저장. 발송은 취소 불가 부작용이라 트랜잭션 밖에서 수행하고 성공한 회원만 기록한다 — 발송과 기록 사이 크래시 창은 그 회원 1명 재발송(at-least-once)이다.
...
- 작성 자격: 인증된 회원이면 작성할 수 있다. 도메인 자체는 회원 자격 상태(정지·탈퇴) 검증이나 상품 존재 검증을 하지 않는다 — cart·coupon·order가 공통으로 요구하는 "자격 활성 회원" 게이트가 이 도메인에는 없다. 이 검증(자격 활성 회원·상품 실존)은 앱 계층 파사드가 조율한다 — 도메인은 본문 검증만 소유한다.
...
응답 유실·프로세스 중단으로 REQUESTED에 남은 결제를 확정하고, 종결 기록된(비REQUESTED) 결제가 남긴 주문측 잔여를 마저 종결하는 흐름(파사드, 주기 스윕·웹훅 공용). 소재는 배치 앱(app-batch)이다 — 스케줄 주기·유예 프로퍼티는 `REQUIREMENTS.md` 제약·전제가, 분산 락 근거는 `decisions.md` ADR-2가 소유한다.
...
- 구현: `order`↔`member`를 `member.id = order.memberId` 논리 조인(물리 FK 없음)으로 직접 잇는다. `fulfillmentStatus`는 `order`↔`fulfillment`(별도 애그리거트)를 `fulfillment.orderId = order.id`로 LEFT JOIN해 채운다 — 이행 행이 없는 주문(미개시)도 검색 결과에서 빠지면 안 되므로 INNER가 아니라 LEFT이며, 행이 없으면 NOT_STARTED로 합성한다. member 도메인의 리포지토리·엔티티에 order 모듈이 컴파일 의존하지 않도록 조회 모듈이 관련 도메인의 쿼리 타입에만 접근한다(모듈 구조는 `docs/architecture.md` 소유).
...
- 도메인 이벤트(`OrderPaid`·`StockRestocked`)는 아웃박스 행 기록이 발행 주체의 커밋과 원자적이라 커밋-발행 사이 유실 창이 없다. 릴레이 전달은 at-least-once이고 소비는 멱등이다(장바구니는 집합 제거의 구조적 멱등, 재입고 알림은 (eventId, memberId) dedup). 재시도 상한 초과분은 dead letter로 격리돼 운영 개입 전까지 미전달로 남는다 — 완전한 무손실은 아니되, 미전달의 영향은 장바구니 잔존·알림 미발송뿐이라 핵심 정합(돈·재고·주문 상태)을 깨지 않는다.
...
- 소비: wishlist 도메인 리스너 — 해당 상품을 찜한 미탈퇴 회원에게 재입고 알림 메일을 발송한다. 집합 제거처럼 자연 멱등이 아니라 처리 기록(`RestockNotification`의 (eventId, memberId) 조합 유니크)을 명시적 dedup으로 써 재전달·부분 실패 재시도의 회원별 중복 발송을 막는다(소비 정책 상세는 §8 소유).
...
| wishlist | 상태 없음(찜은 물리 삭제, 알림 처리 기록은 (eventId, memberId) 유니크) |
