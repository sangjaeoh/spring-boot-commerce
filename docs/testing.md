...
- 경계 케이스는 단위·슬라이스가 소유한다.
- 아키텍처 테스트는 행동 테스트와 섞지 않는다.

### 배치·네이밍

- 테스트는 대상 모듈의 `src/test`에 둔다.
- 패키지 미러링·픽스처 소유는 → [architecture](architecture.md)의 패키지 네이밍·배치.
- 통합 테스트는 해당 앱 모듈의 `src/test`가 소유한다.
- `module-tests/`에 통합 테스트 모듈을 만들지 않는다.
- 테스트 클래스는 `{대상}Test`로 이름 짓는다.
- 행동 그룹은 `@Nested` 클래스로 묶는다.
- `@DisplayName`은 조건과 결과를 담은 한국어 서술문으로 작성한다.
- 테스트 메서드명은 영어 camelCase 요약으로 짓는다.
- 명세는 `@DisplayName`이 소유한다.
...
- 전수 열거는 `@ParameterizedTest` 하나로 묶는다.
...
- 시간 경계는 시각을 파라미터·주입으로 받는 경로에서 검증한다.
- `now()` 직접 호출 경로는 결정성의 구간 단언을 따른다.
...
- 프레임워크 자체의 동작은 테스트하지 않는다.
- 프레임워크에 건 선언(검증 애노테이션·쿼리 메서드·매핑)이 의도와 일치하는지는 테스트한다.
- 필드를 그대로 반환하는 getter·단순 위임은 테스트하지 않는다.
- 커버리지 수치는 게이트가 아니다.
- 합의된 시나리오 충족이 기준이다.
- 테스트만을 위한 프로덕션 공개 표면 확장을 금지한다.
- 접근제한자 완화 금지는 → [coding-conventions](coding-conventions.md).
...
- 테스트 도구는 아래로 고정한다.
- 버전은 Spring Boot BOM과 `gradle/libs.versions.toml`이 소유한다(→ [code-quality](code-quality.md)의 버전).
