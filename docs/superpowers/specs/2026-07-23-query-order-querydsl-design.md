...
- `gradle/libs.versions.toml`: `querydsl = "7.5"` 버전과 라이브러리 추가 — `querydsl-core`, `querydsl-jpa` (좌표 `io.github.openfeign.querydsl`). `querydsl-apt`는 classifier(`jakarta`)가 필요한데 TOML이 classifier를 지원하지 않아 컨벤션 플러그인에서 문자열 좌표로 선언한다.
- `convention.domain-module.gradle.kts`:
  - `annotationProcessor("io.github.openfeign.querydsl:querydsl-apt:{버전}:jakarta")` — 도메인 모듈 컴파일 시 Q클래스 생성, jar 포함. 프로세서 경로 의존(codegen·jakarta.persistence-api·jakarta.inject-api)은 pom 전이로 해석되므로 명시 추가하지 않는다.
  - `implementation(querydsl-core)` — 생성된 Q클래스가 참조하는 타입 컴파일에 필요.
- `module-common/common-jpa/build.gradle.kts`: 같은 apt + `implementation(querydsl-core)` 추가 — apt는 바이너리(의존성 jar) 상위타입의 Q타입을 생성하지 않으므로, `BaseTimeEntity` 소유 모듈이 `QBaseTimeEntity`를 생성해 jar로 배포해야 도메인 Q타입이 컴파일된다(스크래치패드 apt 실험으로 실증).
- `convention.java-base.gradle.kts`: ErrorProne `excludedPaths`로 apt 생성 소스(`build/generated/sources/annotationProcessor`)를 NullAway·ErrorProne 게이트에서 제외한다 — 생성 코드는 게이트 대상이 아니다.
