...
- 앱 모듈은 아래 서브 패키지로 구성한다.
  ...
  - 관객별 인가는 앱 소유 메타 애노테이션으로 표시한다 — `@Admin`(관리자 전용)·`@Authenticated`(로그인 전용)·`@Anonymous`(익명 전용). `com.commerce.api.web.auth`가 소유하고 각각 `@PreAuthorize`를 합성하며 클래스·메서드 레벨 모두에 붙는다(`@Target({TYPE, METHOD})`).
...
- 외부 부작용(PG 청구 등)을 내는 쓰기는 그 부작용을 트랜잭션 밖에서 수행하고 결과 영속만 프로그램적 `TransactionTemplate`으로 좁힌다. 영속이 실패하면 부작용을 보상한다(청구 실패 시 환불).
  - 부작용을 `@Transactional` 안에 두면 커밋 실패 시 부작용이 롤백 없이 고아가 된다(청구는 됐는데 결제 행은 롤백). 이 경우만 애노테이션 대신 프로그램적 트랜잭션을 쓰고, 호출자는 이 메서드를 트랜잭션 밖에서 부른다(안이면 `REQUIRED`가 합류해 부작용이 다시 트랜잭션에 묶인다). 한 트랜잭션 하나의 애그리거트는 유지된다(영속은 자기 애그리거트만 바꾼다).
