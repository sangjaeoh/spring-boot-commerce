...
- 형식은 내용이 정한다. 요약·계약은 Javadoc 블록(`/** */`), 구현의 "왜"는 라인 주석(`//`).
...
- 라인 주석(`//`)은 비자명한 "왜"만 적는다. 코드가 말하는 What 되풀이 금지.
  - 예외는 조율 메서드의 단계 표시다. 순서 자체가 정보일 때 `// 1. 입력 검증`처럼 번호를 단다.
    - 한 단계는 한 줄 명사구. 단계가 셋 미만이거나 순서를 바꿔도 되면 달지 않는다.
  - 시점 없는 사실로 쓴다("우리가 …를 택했다"가 아니라 "…이므로 …한다").
  - 기계적으로 어려운 블록은 주석보다 리팩터·이름 개선을 먼저 한다. 이름이 담을 수 없는 도메인·외부 규정의 "왜"만 인라인으로 남긴다.
