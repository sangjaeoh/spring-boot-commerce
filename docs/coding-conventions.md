...
- 커스텀 예외는 경계에 도달하는(핸들러까지 전파되는) 예외만 `BaseException`을 상속한다. 국소에서 잡아 처리하는 예외는 JDK 관용구(`IllegalArgumentException` 등)로 둔다.
  - 값 범위·불변식 위반은 설계 오퍼레이션 표의 "거부"에 오르면(클라이언트·관리자 입력이 도달) `{Name}ErrorCode` 도메인 예외로, 거부에 없는 호출자 보장 선행조건이면 `IllegalArgumentException`으로 둔다. 후자가 propagate하면 서버 버그(500)이지 클라이언트 400이 아니다.
    - 원생 외부 입력은 경계(요청 DTO 검증·타입된 입력)에서 먼저 걸러 도메인의 선행조건 검사가 명백한 버그 백스톱(IAE)이 되게 한다 — 그래야 새 호출 경로가 IAE를 클라이언트 유발 500으로 바꾸는 비국소 함정이 준다.
...
- 요약은 타입·메서드·영속 필드·enum 상수에 둔다.
  - 타입 요약은 그 타입이 무엇인지만 적는다.
    - 상태 전이 규칙·동시성 메커니즘·설계 근거는 적지 않는다. enum 상수·메서드 doc·도메인 모델 문서가 각각 소유한다 → [`DOMAIN_MODEL.md`](../DOMAIN_MODEL.md)
    - 개별 멤버가 하는 일은 멤버 doc이 소유한다. 같은 사실을 양쪽에 두면 각각 드리프트한다.
    - 이름이 곧 무엇인지 말하는 타입은 타입 doc이 없을 수 있다.
  - 메서드 요약은 공개면 호출자 계약(무엇을 보장하나), 내부면 그 메서드가 맡은 일을 적는다.
  ...
  - 필드 요약은 그 값이 무엇인지만 적는다. 형제 필드와의 구분, 단위·스케일·시간대, 값 범위·존재 조건, 다른 필드와의 산식이 대상이다.
    ...
    - 대상은 `@Entity`·`@Embeddable`의 영속 필드다. record면 필드 자리가 없으므로 `@param`으로 적는다(→ 아래 표기 규칙).
    - 컬럼 코멘트(`columnDefinition`·`@Comment`)로 대신하지 않는다. 스키마를 마이그레이션이 소유하면 DDL 생성 경로가 없어 무효과다 → [architecture](architecture.md)
...
- 이 절은 테스트 소스에도 적용한다. 테스트 메서드만 `@DisplayName`으로 대신하고, 하네스·스텁·픽스처 타입은 같게 요약을 둔다.
- Javadoc은 API 계약이다. 구현 방식(How)은 적지 않는다.
  - 문서화 대상 자신의 계약만 적는다.
    - 관측 가능한 계약(전파 예외·외부 효과)은 협력자를 거쳐 실현돼도 적는다. 협력자의 내부 동작·다른 요소의 계약은 되풀이하지 않는다.
    - 재사용 타입(enum·VO) doc에 그 값이 어느 필드에서 언제 존재하는지 적지 않는다. 그건 그 필드의 계약이다.
  - Javadoc 구조(요약·태그 유효성)는 Error Prone이 경고로 리포트한다(빌드 게이트 아님) → [code-quality](code-quality.md)
- 형식은 내용이 정한다. 요약·계약은 Javadoc 블록(`/** */`), 구현의 흐름·이유는 라인 주석(`//`).
  - 타입 요약은 계사문(`주문 애그리거트 루트다`), 메서드 요약은 3인칭 서술형(`사용자를 등록한다`), 필드·enum 상수 요약은 명사구(`최초 등록 시각`). 마침표로 끝낸다.
  - 명령형(`~하라`)·장황한 주어형(`이 메서드는 …을 반환한다`) 금지.
