...
- Javadoc은 API 계약이다. 공개(`public`·`protected`) 타입·멤버에 호출자가 알아야 할 계약(무엇을 보장하나)을 적고 구현 방식(How)은 적지 않는다.
  ...
  - 명령형(`~하라`)·장황한 주어형(`이 메서드는 …을 반환한다`)은 피한다.
  - 주석은 문서화 대상 자신의 계약만 적는다. 협력자를 거쳐 실현되더라도 대상의 관측 가능한 계약(전파 예외·외부 효과)은 적되, 협력자의 내부 동작이나 다른 요소의 계약은 되풀이하지 않는다.
    - 재사용 타입(enum·VO)의 doc에는 그 타입 자신의 의미만 적는다. 그 값이 어느 필드에서 언제 존재하는지(`{@code SUSPENDED}에서만 존재`)나 인접 필드가 무엇을 담는지(`탈퇴는 {@code deletedAt}으로 표현`)는 그 필드·엔티티의 계약이므로 타입 doc에 적지 않는다.
  - Javadoc 구조(요약·태그 유효성)는 Error Prone이 경고로 리포트한다(빌드 게이트 아님) → [code-quality](code-quality.md)
  - 컨트롤러 엔드포인트·DTO처럼 Swagger 애노테이션(`@Operation`·`@Schema`)이 API 계약을 소유하면, 애노테이션을 단일 출처로 삼고 같은 내용을 Javadoc으로 중복하지 않는다. 애노테이션에 없는 계약만 Javadoc에 남긴다.
- 요약·주석 의무는 접근제한자가 정한 청중이 가른다. 공개 멤버는 미지의 호출자를 위한 계약이라 요약을 기본으로 두고, 내부 멤버는 정보격차가 있을 때만 둔다.
  - 공개(`public`·`protected`)는 시그니처가 계약 전부를 말할 때만 요약을 생략한다. 이름·반환 타입 외에 전파 예외·부수효과·경계동작(비자명한 입력 처리·반환 규약)이 없으면 자기서술이다. 하나라도 있으면 요약을 두고, 그 항목이 요약의 내용이다.
    - 정상 호출자도 마주칠 수 있는 전파 예외(거부·낙관락 충돌 등)는 `@throws`로 적는다. 호출자 보장 선행조건 위반으로만 나는 예외(버그 백스톱)는 계약이 아니라 적지 않는다.
    - 자명한 접근자·오버라이드, 그리고 이름이 곧 동작이며 예외·부수효과가 없는 파생(`totalOf`·`hasStock`)은 생략한다. 이름을 되풀이할 뿐인 요약은 두지 않는다.
  - 내부(`private`·package-private)는 이름과 지역 코드가 도메인 의도를 못 담거나, 비자명한 트레이드오프·불변식을 택하거나, 외부 제약·순서 의존이 동작을 지배할 때만 주석을 둔다. 이름이 의도를 담는 기계적 단계에는 두지 않는다.
- 형식은 내용이 정한다. 호출자 계약은 Javadoc 블록(`/** */`)에, 구현의 "왜"는 접근제한자와 무관하게 라인 주석(`//`)에 둔다.
  - Javadoc 블록은 도구가 호출 지점에 계약을 띄우고 `{@link}`·`@throws`를 타입에 연결한다.
- 조감도(타입이 무엇을 조율·표현하나)는 타입 doc이 소유한다. 멤버 doc은 자기 계약만 적고 타입 맥락을 되풀이하지 않는다.
  - 맥락을 멤버마다 되풀이하면 같은 사실이 여러 곳에서 각각 드리프트한다. 조율·불변식이 얇은 타입은 타입 doc이 없을 수 있다.
- 구현 주석은 비자명한 "왜"만 적는다. 코드가 이미 말하는 것(What)을 되풀이하지 않는다.
  - 시점 없는 사실로 쓴다("우리가 …를 택했다"가 아니라 "…이므로 …한다").
  - 기계적으로 어려운 블록은 주석보다 리팩터·이름 개선을 먼저 한다. 이름이 담을 수 없는 도메인·외부 규정의 "왜"는 인라인으로 남긴다.
- 표기는 코드·값은 `{@code …}`, 타입은 `{@link …}`.
  - 반환·인자는 요약 문장에 녹인다. `@return`과 자명한 `@param`은 두지 않고, `@param`은 record 컴포넌트나 비자명한 제약(조건부 존재·범위)에만 붙인다.
- 주석에 넣지 않는 것:
