...
- 주석은 한국어로 쓴다.
  - 용어집에 등재된 용어만 쓴다 → [`DOMAIN_MODEL.md`](../DOMAIN_MODEL.md). 같은 영문 개념을 주석마다 다르게 부르면 어휘가 갈린다. 미등재 개념은 용어집에 먼저 올린다.
  - 도메인 개념이 타입 doc에 처음 나올 때 `한국어(영문식별자)`로 병기한다. 멤버 doc·라인 주석에는 병기하지 않는다.
- 요약은 타입·메서드·영속 필드·enum 상수에 둔다. 각각 한 문장이다.
  ...
  - 필드 요약은 그 값이 무엇인지만 적는다. 형제 필드와의 구분, 단위·스케일·시간대, 값 범위·존재 조건, 다른 필드와의 산식이 대상이다.
    ...
    - 대상은 `@Entity`·`@Embeddable`의 영속 필드다. record면 필드 자리가 없으므로 컴포넌트마다 `@param`으로 적는다.
    - 컬럼 코멘트(`columnDefinition`·`@Comment`)로 대신하지 않는다. 스키마를 마이그레이션이 소유하면 DDL 생성 경로가 없어 무효과다 → [architecture](architecture.md)
  - enum 상수 요약은 의미를 적고, 상태 enum이면 진입 조건을 더한다. 상수당 한 줄을 넘기지 않는다.
    - 전이 전체는 도메인 모델 문서가 소유한다 → [`DOMAIN_MODEL.md`](../DOMAIN_MODEL.md)
- 요약을 두지 않는 자리는 다른 데가 그 의미를 이미 소유하는 자리다.
  - 상위 계약을 그대로 따르는 오버라이드 — 상위 doc.
  - 필드 반환 한 줄 접근자(`get*`·`is*`) — 필드 doc.
    - 계산·파생·방어적 복사가 섞이면 접근자가 아니다. `has*`·`can*`는 항상 요약을 둔다.
...
- 형식은 내용이 정한다. 요약·계약은 Javadoc 블록(`/** */`), 구현의 흐름·이유는 라인 주석(`//`).
  - 타입·메서드 요약은 3인칭 서술형(`사용자를 등록한다`), 필드·enum 상수 요약은 명사구(`최초 등록 시각`). 마침표로 끝낸다.
  - 명령형(`~하라`)·장황한 주어형(`이 메서드는 …을 반환한다`) 금지.
  - Javadoc 블록은 도구가 호출 지점에 계약을 띄우고 `{@link}`·`@throws`를 타입에 연결한다.
- 라인 주석(`//`)은 조율 메서드의 단계 표시와, 코드가 답하지 못하는 이유를 적는다.
  ...
  - 이유는 단계 위에 덧붙인다. 왜 이 순서인지, 왜 이 값인지, 왜 손대지 않는지 — 코드가 답할 수 없는 것.
  - 자명한 한 줄을 한국어로 옮겨 적지 않는다. 이름이 이미 말한 것을 되풀이하면 코드가 바뀔 때 한쪽이 거짓이 된다.
  - 시점 없는 사실로 쓴다("우리가 …를 택했다"가 아니라 "…이므로 …한다").
  - 기계적으로 어려운 블록은 주석보다 리팩터·이름 개선을 먼저 한다.
- 표기는 코드·값 `{@code …}`, 타입 `{@link …}`.
  - 반환·인자는 요약 문장에 녹인다. `@return`과 자명한 `@param` 금지.
  - `@param`은 record 컴포넌트나 비자명한 제약(조건부 존재·범위)에만 붙인다.
    - 엔티티 필드를 그대로 옮긴 컴포넌트는 엔티티 doc이 소유하므로 중복 금지. 엔티티에 없는 파생·합성 컴포넌트만 적는다.
- 주석에 넣지 않는 것:
