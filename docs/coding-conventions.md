...
- 예외: 모듈 밖 계약인 `public static final` 상수는 허용한다(캐시 이름 상수 등).
...
- 용어집 문서가 없으면 위치·생성부터 사람에게 합의로 요청한다.
...
- 예외: 캐시 미스 경로 위임 서비스는 계약 없이 `Transactional{계약명}`으로 짓는다 → [caching](caching.md)의 트랜잭션 분리.
...
- 파생 쿼리 메서드의 활성-only 조회는 이름에 `DeletedAtIsNull`을 담는다.
- `@Query`·QueryDSL의 활성-only 조회는 활성 필터를 쿼리 본문이 소유한다.
- 쿼리 본문 소유 조회는 이름에 활성 접미사를 담지 않는다.
- 삭제 포함 조회는 방식과 무관하게 이름에 `IncludingDeleted`를 붙인다.
- 사실조회형(상태·속성 질의) boolean 메서드는 `is*`, `has*`, `can*`를 사용한다.
- 시도·명령형 boolean 반환(성공 여부 신호 등)은 접두사를 자유롭게 정한다.
...
| `Exception` | 커스텀 예외 |
| `Gateway` / `Store` | required 계약(외부 시스템·영속) |
| `Publisher` | 이벤트 발행 계약(소유는 → [architecture](architecture.md)의 크로스 도메인 상호작용) |
| `Filter` | 서블릿 필터 |
| `Listener` | 이벤트 리스너 |
| `Config` | 설정 |
| `Aspect` | AOP |
