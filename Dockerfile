# app-migration·app-api·app-admin·app-batch 공용 멀티스테이지 빌드. compose의 build.target으로 앱을 고른다.
...
RUN --mount=type=cache,target=/root/.gradle \
    ./gradlew --no-daemon :module-apps:app-migration:bootJar :module-apps:app-api:bootJar :module-apps:app-admin:bootJar :module-apps:app-batch:bootJar
...
FROM runtime AS app-admin
COPY --from=build /workspace/module-apps/app-admin/build/libs/app-admin.jar /app/app.jar
EXPOSE 8080
ENTRYPOINT ["java", "-jar", "/app/app.jar"]

FROM runtime AS app-batch
COPY --from=build /workspace/module-apps/app-batch/build/libs/app-batch.jar /app/app.jar
EXPOSE 8080
ENTRYPOINT ["java", "-jar", "/app/app.jar"]
