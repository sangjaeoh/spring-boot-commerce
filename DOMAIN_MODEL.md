...
| productId | UUID | 필수 | 리뷰 대상 상품 참조(product 도메인 논리 참조). (memberId, productId) 유니크 |
| rating | int | 필수 | 별점 1~5 |
| content | String | 필수 | 본문 1~1000자(공백만인 본문 거부) |
| createdAt | Instant | 필수 | 생성 시각(Auditing 자동 기록). 목록 정렬의 "작성 시각"으로 노출 |
| updatedAt | Instant | 필수 | 수정 시각(Auditing 자동 기록) |
| deletedAt | Instant | 선택 | 관리자 제거 시각. 관리자 제거된 리뷰만 값을 가짐(본인 삭제는 물리 삭제라 행 자체가 없어짐) |
| removedReason | String | 선택 | 관리자 제거 사유. 관리자 제거된 리뷰만 값을 가짐 |

- version(낙관락) 없음 — 동시 작성 경합은 유니크 인덱스 + 사전 exists 검사로 처리하고(아래 정책), 별점·본문 자체는 동시 수정 경합의 대상으로 보지 않는다.

### 상태

전용 상태 enum 없음. `deletedAt` 유무로 활성/관리자 제거 두 상태만 구분한다(활성 조회는 `deletedAt IS NULL`).
...
- (memberId, productId) 유니크 — 상품당 한 건. DB 유니크 인덱스가 최후 방어선이고, 애플리케이션은 저장 전 exists 검사로 사전 거부한다. 이 exists 검사는 관리자 제거(소프트삭제)된 리뷰도 포함한다 — 재작성 불가 정책과 정합.
...
- 관리자 제거와 본인 삭제는 물리적으로 다르다: 관리자 제거는 `deletedAt`·`removedReason`을 채우는 소프트삭제이고, 본인 삭제는 물리 DELETE다. 이 차이가 재작성 가능 여부를 가른다 — 본인 삭제는 행이 사라져 유니크·exists 검사에 걸리지 않으므로 같은 상품에 재작성할 수 있고, 관리자 제거는 행이 남아(exists 검사가 포함) 재작성이 영구 차단된다.
...
| 연산 | 입력 | 강제 불변식 | 거부 |
|---|---|---|---|
| write(작성 자격 게이트는 파사드 경유) | memberId, productId, rating, content | 배송완료(PAID·DELIVERED) 구매 이력 존재(파사드 사전 검증); (memberId, productId) 유니크(관리자 제거 이력 포함 사전 exists 검사 + REQUIRES_NEW 격리 + 유니크 인덱스 최후 방어); rating 1~5; content 1~1000자(공백만 거부) | 배송완료 이력 없음(파사드 소유); 중복 작성; 별점·본문 범위 위반 |
| revise | reviewId, memberId, rating, content | 본인 소유 ∧ 미제거 리뷰만 대상; rating·content 같은 검증 | 미존재(타인 소유·이미 관리자 제거 포함); 범위 위반 |
| remove(본인 삭제) | reviewId, memberId | 본인 소유 ∧ 미제거 리뷰만 대상; 물리 삭제(재작성 가능해짐) | 미존재(타인 소유·이미 관리자 제거 포함) |
| removeByAdmin | reviewId, reason | 미제거(deletedAt null) 리뷰만 대상; 소유 무관; 소프트삭제(`deletedAt`·`removedReason` 기록, 재작성 영구 차단) | 미존재(이미 제거 포함) |
...
- 작성 자격: 인증된 회원이면 작성할 수 있다. 회원 자격 상태(정지·탈퇴) 검증이나 상품 존재 검증을 하지 않는다 — cart·coupon·order가 공통으로 요구하는 "자격 활성 회원" 게이트가 이 도메인에는 없다.
...
| review | 전용 상태 없음(관리자 제거는 deletedAt, 본인 삭제는 물리 삭제) |
