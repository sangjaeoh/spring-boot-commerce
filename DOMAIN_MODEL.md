...
- 환원 불가능한 잔여: 다중 라인 주문의 재고 복원 루프 중간 실패는 재시도가 이미 복원된 라인을 가산 재복원한다. 성공 완결 후 요청 재전송으로 인한 중복 취소 호출은 재고를 이중 복원한다(예외-재시도가 아닌 재호출의 멱등은 HTTP 경계 책임으로 범위 밖). durable한 라인별 복원 진행 비트가 없어(주문 PAID/CANCELLED가 유일 신호) 라인별 exactly-once는 불변식 안에서 도달 불가다.
...
- `@Version`: `stock`·`issued_coupon`에 두고 `payment`·`product_variant`에는 두지 않는다. version 컬럼 추가·`ddl-auto=validate` 정합은 `docs/entity-persistence.md`가 소유한다.
